...
function  getSetting(){
    $ci = &get_instance();
    $ci->load->database();
    $settings = $ci->db->select('*')->where('id',5)->get('setting')->result_array();
    return $settings[0];
...
$mail = new PHPMailer(true);
    ...
    // $mail = new PHPMailer();
        ...
        $mail->AddReplyTo($smtp['SMTPFROMEMAIL'], $smtp['SMTPFROMNAME']);
...
function getClientName($clientID) {
    $ci = &get_instance();
    $ci->load->database();
    $category = $ci->db->select("ca_fname")->where("ca_id", $clientID)->get('client')->result_array();
    if (count($category) > 0) {
        $getKeyquery = $category;
        return $getKeyquery[0]['ca_fname'];
    }else{
        return 0;
    }
}
...
function getAllInvoice() {
    $ci = &get_instance();
    $ci->load->database();
    $category = $ci->db->select("dbRowId, InvoiceID")->where("deleted", 'N')->get('db')->result_array();
    if (count($category) > 0) {
        $getKeyquery = $category;
        return $category;
    }else{
...
function getAllProduct() {
    $ci = &get_instance();
    $ci->load->database();
    $product = $ci->db->select("id,name,unit_price")->where("virtual_status", 'ACTIVE')->get('product')->result_array();
    if (count($product) > 0) {
        $getKeyquery = $product;
        return $product;
    }else{
...
function getAllClient() {
    $ci = &get_instance();
    $ci->load->database();
    $category = $ci->db->select("ca_id,ca_fname")->where("ca_virtualStatus", 'ACTIVE')->get('client')->result_array();
    if (count($category) > 0) {
        $getKeyquery = $category;
        return $category;
    }else{
...
function getClientNotifyList() {
    $ci = &get_instance();
    $ci->load->database();
    $ci->db->select('customers.*');
    $ci->db->from('customers');
    $ci->db->where('customers.deleted', 'N');
    $ci->db->where("customers.notify", 'yes');
    $ci->db->where_not_in('customers.email', '');
    $ci->db->order_by('customers.customerRowId desc');
    $query =$ci->db->get();
