...
class Client_model extends CI_Model {
    ...
    public function addClient_ctrl($fields){
        $result = $this->db->insert('client', $fields);
    ...
    public function getAllClient(){
        ...
        $result = $this->db->select('*')->where($where)->order_by('ca_id', 'DESC')->get('client')->result_array();
        return $result;
    } 
    public function updateNotify($fields, $where) { 
            $result = $this->db->where('ca_id', $where)->update('client', $fields);
            return $result;
    }
   ...
   public function deleteclient(){
    ...
    $deleteclient = $this->db->update('client', $deleteArray);
    return $deleteclient;
}
...
public function updateClient($fields,$where){
    $result = $this->db->where($where)->update('client', $fields);
    return $result;
} 
...
{
 $where = array('ca_fname' => $getname, 'ca_virtualStatus' => 'ACTIVE');
 $result = $this->db->select('*')->where($where)->get('client')->result_array();
 return $result;
}
public function editNameExistsCheck($getname,$id)
{
 $where = array('ca_fname' => $getname,'ca_virtualStatus' => 'ACTIVE');
 $id = base64_decode($id);
 $result = $this->db->select('*')->where($where)->where('ca_id !=',$id)->get('client')->result_array();
 return $result;
}
}     