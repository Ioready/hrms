...
class Sale_model extends CI_Model 
{
    ...
    {
        $this->db->select('dbdetail.*, items.*, db.netAmt as grandTotal,db.InvoiceID as InvoiceID,db.dispatchNo, db.orderID as orderID, db.remarks as Remarks' );
        $this->db->from('dbdetail');
        $this->db->where('dbdetail.type !=', 'quote');
        $this->db->where('dbdetail.dbRowId', $invNo);
        $this->db->join('db','db.dbRowId = dbdetail.dbRowId');
        $this->db->join('items','items.itemRowId = dbdetail.itemRowId');
        $this->db->order_by('dbdRowId');
        $query = $this->db->get();

        return($query->result_array());
    }
    public function getProducts_forQuote($invNo)
    {
        $this->db->select('dbdetail.*, items.*, db.netAmt as grandTotal,db.InvoiceID as InvoiceID,db.dispatchNo, db.orderID as orderID, db.remarks as Remarks' );
        $this->db->from('dbdetail');
        $this->db->where('dbdetail.dbRowId', $invNo);
	...
	public function insert()
    {
        ...
        $data = array(
            ...
            , 'invoiceID' => generateRandomCode()
            , 'dispatchNo' => generateRandomCode()
    ...
    public function getDataLimit_forQuote()
    {
        $this->db->select('db.*, customers.customerName');
        $this->db->where('db.deleted', 'N');
        $this->db->where('db.type', 'quote');
