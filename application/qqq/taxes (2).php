                                    ...
                                    <label class="switch">
                                        ...
                                        <span class="slider round"></span>
                                          ...
                                          <td>
                                            ...
                                            <a data-toggle="modal" onclick="setDeleteClientId('<?php echo base64_encode($value['id']); ?>');" data-target="#deleteTaxModal"><i class="fa fa-trash" data-toggle="tooltip" data-original-title="Delete User" data-placement="left" title=""></i></a>
                                          </td>
                                        </tr>
                                      <?php }} ?>
                                    </tbody>
...
<div class="modal fade issuerpopup" id="deleteTaxModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- Wait Alert -->
            <div id='deleteTaxwait_msg' class="alert alert-warning no-border" style="display: none;">
                <button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button>
                <span class="text-semibold"><b>Please Wait! </b>Your data is validating</span>
            </div>
            <!-- Error Alert -->
            <div id='deleteTaxerror_msg' class="alert alert-danger no-border" style="display: none;">
                <button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button>
                <span class="text-semibold"><b>Sorry! </b></span><span id="deleteTaxerror_content"></span>
            </div>
            <!-- Success Alert -->
            <div id='deleteTaxsuccess_msg' class="alert alert-success no-border" style="display: none;">
                <button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button>
                <span class="text-semibold"><b>Success! </b></span><span id="deleteTaxsuccess_content"></span>
            </div>
            <form class="form-horizontal" method="post" name="deleteTax" id="deleteTax">
...
<script>
    ...
    $("#addtaxform").submit(function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result == 1) {
                                ...
                                window.scrollTo(0, 0);
                                $('#sendingPasswordErrorContent').html('Name Already Exists!..');
        ...
        e.preventDefault();
        window.scrollTo(0, 0);
        $('#deleteTaxwait_msg').slideDown(1000);
        var data = $(this).serialize();
        $.ajax({
            ...
            success: function(result) {
                if (result == 1) {
                    window.scrollTo(0, 0);
                    $('#deleteTaxsuccess_content').html("Tax deleted successfully");
                    $('#deleteTaxwait_msg').slideUp(1000);
                    $('#deleteTaxsuccess_msg').slideDown(1000);
                    setTimeout(function() {
                        window.location.href = baseURL + 'taxes';
                    }, 4000);
                } else {
                    window.scrollTo(0, 0);
                    $('#deleteTaxerror_content').html(result);
                    $('#deleteTaxwait_msg').slideUp(1000);
                    $('#deleteTaxerror_msg').slideDown(1000);
                    setTimeout(function() {
                        $('#deleteTaxerror_msg').slideUp(1000);
                    }, 4000);
                }
            },
        });
