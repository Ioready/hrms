...
// }
			...
			var discount = $("#txtTotalDiscount").val();
				...
				if (discount > 0){
					...
					Final_gross = parseFloat(gross_price) + parseFloat(result);
					$("#gross_price").val(Final_gross.toFixed(2));
				// $("#gross_price").val(Final_gross.'+ '.discount.toFixed(2));
				}else{
			...
			function calcBalNow()
			{
				if( $("#txtAmtPaid").val() !== "" )
				{
					// $("#txtBalance").val( (parseFloat($("#gross_price").val()) + $("#txtAmtPaid").val()).toFixed(2));
  ...
  <script type="text/javascript">
	...
	function printThis(printrow)
	{
		...
		globalrowid = $(this).closest('tr').children('td:eq(2)').text();
      	...
      	$.ajax({
			'url': base_url + '/' + controller + '/printPDF',
			'type': 'POST', 
			'data':{ 'globalrowid':globalrowid },
			'dataType': 'json',
			'success':function(data)
